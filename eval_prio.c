...
//Question 6
/** Recherche l'opérateur de plus faible priorité */
int is_weaker(struct token* curr, struct token* opp){
	if(opp == NULL) return 1;

	if(!strcmp(curr->content.op, "<=>")){
		return 1;
	}
	if(!strcmp(opp->content.op, "<=>")){
		return 0;
	}
...
liste_token chrch_opp(liste_token lt, int nb){
	...
	while(curr && i > 0){
		if(curr->type == TOKEN_OP && (entre_par == 0)){
			//L'opérateur le moins prioritaire est le dernier lu si c'est le même que son
			//prédécesseur, sinon is_weaker permet de déterminer la priorité
			if(opp == NULL || 
			((strcmp(opp->content.op, curr->content.op) != 0) && is_weaker(curr, opp))){
				opp = curr;
			}
		}
